...
def hyperparameter_tuning():
  with open("ML-Test_Repo/gru_logs.txt", 'w') as logfile:
    ...
    for sequence_length, learning_rate, num_epochs, hidden_size in itertools.product(sequence_lengths,
      ...
      train_dates = index_data_obj.index_data[:len(x_train)].iloc[:,:1].values
      print(train_dates)
      test_dates = index_data_obj.index_data[len(x_train):len(x_train)+len(x_test)].iloc[:,:1].values
      train_indices = index_data_obj.index_data[:len(x_train)].iloc[:,2:].values
      print(train_indices)
      test_indices = index_data_obj.index_data[len(x_train):len(x_train)+len(x_test)].iloc[:,2:].values
